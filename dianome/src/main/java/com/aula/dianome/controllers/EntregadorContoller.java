...
@RestController
public class EntregadorContoller {
	...
	public ResponseEntity<Object> deleteProduct(@PathVariable(value="id") Integer id,@RequestBody @Valid EntregadorDto dto){
	Optional<EntregadorModel> entregador = entregadorRepository.findById(id);
	if(entregador.isEmpty()) {
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Entregador não encontrado!");		
	}
	entregadorRepository.delete(entregador.get());
	return ResponseEntity.status(HttpStatus.OK).body("Entregador foi excluido!");
	}
}
